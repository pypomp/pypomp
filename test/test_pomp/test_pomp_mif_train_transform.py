...
def test_mif_traces_transformed(simple_pomp_with_transform):
    ...
    rw_sd = pp.RWSigma(
        sigmas={k: 0.02 for k in LG.theta.keys()},
        init_names=[],
    )
    ...
    for param in param_names:
        if param.startswith('Q') or param.startswith('R'):
            ...
            assert np.all(param_values > 0), f"Parameter {param} should be positive in natural space"
            ...
            assert np.all(param_values > 0.0001), f"Parameter {param} values seem to be in log space"
...
def test_transform_roundtrip():
    ...
    back_to_nat = par_trans.transform_array(est_space, param_names, direction="from_est")
