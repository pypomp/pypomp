            ...
            for CLL, ESS, filter_mean, prediction_mean in bool_cases:
                with self.subTest(
                    ...
                    if CLL:
                        ...
                        self.assertEqual(
                            condLogLiks_arr.shape, expected_shape + (len(self.ys),)
                        )
                        self.assertTrue(jnp.all(jnp.isfinite(condLogLiks_arr)))
                        self.assertTrue(
                            jnp.issubdtype(condLogLiks_arr.dtype, jnp.floating)
                        )
                    else:
                        with self.assertRaises(KeyError):
                            _ = self.LG.results_history[-1]["CLL"]
                    ...
                    if ESS:
                        ...
                        ess_arr = ess.data
                        self.assertEqual(
                            ess_arr.shape, expected_shape + (len(self.ys),)
                        )
                    ...
                    if filter_mean:
                        ...
                        self.assertEqual(
                            filter_mean_arr.shape, expected_shape + (len(self.ys), 2)
                        )
                        self.assertTrue(jnp.all(jnp.isfinite(filter_mean_arr)))
                        self.assertTrue(
                            jnp.issubdtype(filter_mean_arr.dtype, jnp.floating)
                        )
                    else:
                        with self.assertRaises(KeyError):
                    ...
                    if prediction_mean:
                        ...
                        self.assertEqual(
                            prediction_mean_arr.shape,
                            expected_shape + (len(self.ys), 2),
                        )
                        self.assertTrue(jnp.all(jnp.isfinite(prediction_mean_arr)))
                        self.assertTrue(
                            jnp.issubdtype(prediction_mean_arr.dtype, jnp.floating)
                        )
                    else:
                        with self.assertRaises(KeyError):
