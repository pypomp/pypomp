...
def test_prune(lg_panel_mp):
    ...
    initial_shared_len = (
        len(panel.theta.params(as_list=True)) if panel.theta.params(as_list=True) else 0
    )
    ...
    if original_theta is not None:
        original_shared = [t.get("shared") for t in original_theta.params(as_list=True)]
        original_unit_specific = [
            t.get("unit_specific") for t in original_theta.params(as_list=True)
        ]
    else:
    ...
    assert len(panel.theta.params(as_list=True)) == 1
    ...
    if original_results.method == "mif":
        ...
        if original_shared is not None:
            ...
            current_shared = panel.theta.params(as_list=True)[0].get("shared")
        ...
        if original_unit_specific is not None:
            ...
            current_unit_specific = panel.theta.params(as_list=True)[0].get(
                "unit_specific"
            )
    ...
    assert len(panel.theta.params(as_list=True)) == initial_shared_len
    # All entries should be the same (repeated top replicate)
    if initial_shared_len > 1:
        first_shared = panel.theta.params(as_list=True)[0].get("shared")
        first_unit_specific = panel.theta.params(as_list=True)[0].get("unit_specific")
        for i in range(1, initial_shared_len):
            if first_shared is not None:
                current_shared = panel.theta.params(as_list=True)[i].get("shared")
            ...
            if first_unit_specific is not None:
                current_unit_specific = panel.theta.params(as_list=True)[i].get(
                    "unit_specific"
                )
...
def test_mix_and_match(lg_panel_mp):
    ...
    original_shared = [
        ...
        for t in original_theta.params(as_list=True)
    ]
    original_unit_specific = [
    ...
    panel.mix_and_match()
    assert len(panel.theta.params(as_list=True)) == initial_n_reps

    # Helper to verify a replicate has correct mixed parameters
    def verify_replicate(rep_idx, rank_idx):
        new_shared = panel.theta.params(as_list=True)[rep_idx].get("shared")
        ...
        if original_unit_specific:
            new_spec = panel.theta.params(as_list=True)[rep_idx].get("unit_specific")
    ...
    if (
        ...
        and panel.theta.params(as_list=True)[0].get("unit_specific") is not None
    ):
        new_spec = panel.theta.params(as_list=True)[0]["unit_specific"]
        for unit in unit_names:
