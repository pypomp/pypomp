"""
This module implements likelihood maximization algorithm for POMP models.
"""
import importlib
import pomp_class
...
def fit(pomp_object=None, J=100, Jh=1000, theta=None, rinit=None, rprocess=None, 
        dmeasure=None, rprocesses=None, dmeasures=None, ys=None, sigmas=None, 
        sigmas_init=None, covars=None, M=10, a=0.9, method='Newton', itns=20,
        beta=0.9, eta=0.0025, c=0.1, max_ls_itn=10, thresh_mif=100, thresh_tr=100,
        verbose=False, scale=False, ls=False, alpha=0.1, monitor=True, mode="IFAD"):
    """
    An outside function controlling which fit operation to use for likelihood maximization
    algorithm of POMP model by executing on a pomp class object or by calling function 
    'fit_internal' directly. 


    Args:
        pomp_object (Pomp, optional): An instance of the POMP class. If provided, the function
                                      will execute on this object to conduct the fit operation.
                                      Defaults to None.
        ...
        method (str, optional): The gradient optimization method to use, including Newton 
                                method, weighted Newton, BFGS, gradient descent. 
                                Defaults to 'Newton'.
        ...
        thresh_mif (float, optional): Threshold value to determine whether to resample 
                                      particles in iterated filtering. Defaults to 100.
        thresh_tr (float, optional): Threshold value to determine whether to resample particles
    ...
    Raises:
        ValueError: Missing the required arguments 'sigmas' or 'sigmas_init' in 'IF2' or 
                    'IFAD' when executing on the pomp class object
        ValueError: Invalid mode input when executing on the pomp class object
        ValueError: Missing the required arguments, workhorses, 'sigmas' or 'sigmas_init' 
                    in 'IF2' or 'IFAD' when calling the function 'fit_internal' directly.
    ...
    if pomp_object is not None:
        if mode == "IF2" or mode == "IFAD":
            if sigmas is not None and sigmas_init is not None:
                return pomp_object.fit(sigmas, sigmas_init, M, a, J, Jh, method, itns,
                                       beta, eta, c, max_ls_itn, thresh_mif, thresh_tr,
        ...
        if mode == "IF2" or mode == "IFAD":
            if rprocesses is not None and dmeasures is not None and sigmas is not None and \
                ...
                return _fit_internal(theta, ys, rinit, rprocess, dmeasure, rprocesses, 
                                     ...
                                     method, itns, beta, eta, c, max_ls_itn, thresh_mif, thresh_tr,
                                     verbose, scale, ls, alpha, monitor, mode) 
            ...
            return _fit_internal(theta, ys, rinit, rprocess, dmeasure, rprocesses, dmeasures, 
                                 ...
                                 eta, c, max_ls_itn, thresh_mif, thresh_tr, verbose, scale, ls,
                                 alpha, monitor, mode)
        else:
            raise ValueError("Invalid Mode Input")
    else:
