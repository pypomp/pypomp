...
def _pfilter_helper_complete(t, inputs, rprocess, dmeasure):
    particlesF, theta, covars, loglik, norm_weights, counts, ys, thresh, key, logliks_arr, particles_arr, filter_mean_arr, ess_arr, traj_arr = inputs
    J = len(particlesF)
    ...
    # **save loglik_t (conditional loglikelihood) 和 particlesP (states) **
    ...
    # create filter.traj **
     # variable "counts" records the source particle of particle i (1 - J) at time t from the previous step
    ...
    return [particlesF, theta, covars, loglik, norm_weights, counts, ys, thresh, key, logliks_arr, particles_arr, filter_mean_arr, ess_arr, traj_arr]


def _pfilter_internal_complete(theta, ys, J, rinit, rprocess, dmeasure, covars=None, thresh=100, 
                      key=None):
    ...
    logliks_arr = jnp.zeros(len(ys))  # save loglik_t (conditional loglikelihood)
    ...
    pfilter_helper_2 = partial(_pfilter_helper_complete, rprocess=rprocess, dmeasure=dmeasure)
    particlesF, theta, covars, loglik, norm_weights, counts, ys, thresh, key, logliks_arr, particles_arr, filter_mean_arr, ess_arr, traj_arr = jax.lax.fori_loop(
        lower=0, upper=len(ys), body_fun=pfilter_helper_2,
        init_val=[particlesF, theta, covars, loglik, norm_weights, counts, ys, thresh, key, logliks_arr, particles_arr, filter_mean_arr, ess_arr, traj_arr])
    ...
    for t in range(len(ys)-1, -1, -1):
        filt_traj = filt_traj.at[t].set(particles_arr[t, b])  # extract the state value corresponding the b th particle at time t and plug it into filt_traj
        b = traj_arr[t, b] # find the corresponding particle from the previous step from traj_arr and update b
        b = jnp.int32(b)
    
    # return 1. negative loglikelihood, 
    # 2. mean loglikelihood, 
    # 3. conditional loglikelihood 
    # 4. save states 
    # 5. filter mean
    # 6. effective sample size 
    # 7. filter_traj
    return -loglik, -loglik/len(ys), logliks_arr, particles_arr, filter_mean_arr, ess_arr, filt_traj

def pfilter_complete(pomp_object=None, J=50, rinit=None, rprocess=None, dmeasure=None, theta=None, 
    ...
    if pomp_object is not None:
        ...
        return _pfilter_internal_complete(theta, ys, J, rinit, rprocess, dmeasure, covars, thresh, key)
    else:
