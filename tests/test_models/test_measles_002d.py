from typing import cast

import jax
import jax.numpy as jnp
import numpy as np
import pytest

import pypomp as pp

BASE_THETA_002D = {
    "R0": 56.8,
    "sigma": 28.9,
    "gamma": 30.4,
    "iota1": 1.07,
    "iota2": 0.0,
    "rho": 0.488,
    "sigmaSE": 0.0878,
    "psi": 0.116,
    "cohort": 0.557,
    "amplitude": 0.554,
    "S_0": 2.97e-02,
    "E_0": 5.17e-05,
    "I_0": 5.14e-05,
    "R_0": 9.70e-01,
}

DEFAULT_J = 2
DEFAULT_KEY = jax.random.key(1)


@pytest.fixture(scope="module")
def london_002d():
    measles = pp.models.UKMeasles.Pomp(
        unit="London",
        theta=pp.PompParameters(BASE_THETA_002D.copy()),
        clean=True,
        model="002d",
        last_year=1951,  # Use less data for faster testing
    )
    return measles


def test_002d_simulate(london_002d):
    """Simulated case counts must be non-negative and finite."""
    _, obs = london_002d.simulate(key=DEFAULT_KEY, nsim=1)

    values = obs.drop(columns=["theta_idx", "sim", "time"]).to_numpy()
    assert np.all(np.isfinite(values))
    assert np.all(values >= 0), "measles case counts cannot be negative"


def test_002d_pfilter(london_002d):
    """The filter returns a finite negative log-likelihood."""
    london_002d.pfilter(J=DEFAULT_J, key=DEFAULT_KEY)

    logLiks = np.asarray(london_002d.results_history[-1].payload["logLiks"])
    assert np.all(np.isfinite(logLiks)), f"non-finite logLik: {logLiks}"
    assert np.all(logLiks < 0)


def test_002d_dpop_train(london_002d):
    eta = pp.LearningRate({name: 0.01 for name in london_002d.canonical_param_names})
    london_002d.results_history.clear()
    ret = london_002d._dpop_train(
        J=DEFAULT_J,
        M=2,
        eta=eta,
        optimizer=pp.Adam(),
        alpha=0.8,
        process_weight_state="logw",
        key=DEFAULT_KEY,
    )
    assert ret is None
    res = london_002d.results_history[-1]
    assert res.method == "dpop_train"
    assert not res.traces().empty


def test_002d_par_trans_roundtrip(london_002d):
    """Parameter transform round-trip: natural -> est -> natural."""
    from pypomp.models.measles.model_002d import from_est, to_est

    theta = BASE_THETA_002D.copy()
    theta_jax = cast(dict[str, jax.Array], {k: jnp.array(v) for k, v in theta.items()})
    est = to_est(theta_jax)
    recovered = from_est(est)
    for k in theta:
        assert jnp.allclose(theta_jax[k], recovered[k], atol=1e-3), (
            f"Round-trip failed for {k}: {theta_jax[k]} vs {recovered[k]}"
        )


def test_002d_dmeas_nan_handling():
    """dmeas should return 0 for NaN observations (not NaN)."""
    from pypomp.models.measles.model_002d import dmeas

    Y = {"cases": jnp.array(float("nan"))}
    X = {"C": jnp.array(100.0)}
    theta = {"rho": jnp.array(0.5), "psi": jnp.array(0.1)}
    result = dmeas(Y, X, theta)
    assert jnp.isfinite(result)
    assert result == 0.0


def test_002d_dmeas_zero_cases():
    """dmeas should handle y=0 (uses log_cdf_single path)."""
    from pypomp.models.measles.model_002d import dmeas

    Y = {"cases": jnp.array(0.0)}
    X = {"C": jnp.array(100.0)}
    theta = {"rho": jnp.array(0.5), "psi": jnp.array(0.1)}
    result = dmeas(Y, X, theta)
    assert jnp.isfinite(result)


def test_002d_dmeas_positive_cases():
    """dmeas should handle y>0 (uses log_cdf_diff path)."""
    from pypomp.models.measles.model_002d import dmeas

    Y = {"cases": jnp.array(50.0)}
    X = {"C": jnp.array(100.0)}
    theta = {"rho": jnp.array(0.5), "psi": jnp.array(0.1)}
    result = dmeas(Y, X, theta)
    assert jnp.isfinite(result)
    assert result < 0.0  # log-likelihood should be negative


def test_002d_log_cdf_diff_gradient():
    """log_cdf_diff should produce finite gradients."""
    from pypomp.models.measles.model_002d import log_cdf_diff

    def f(zh, zl):
        return log_cdf_diff(zh, zl)

    zh = jnp.array(1.0)
    zl = jnp.array(-1.0)
    grad_zh, grad_zl = jax.grad(f, argnums=(0, 1))(zh, zl)
    assert jnp.isfinite(grad_zh)
    assert jnp.isfinite(grad_zl)


def test_002d_log_cdf_diff_extreme():
    """log_cdf_diff should handle moderately extreme z values without NaN gradients."""
    from pypomp.models.measles.model_002d import log_cdf_diff

    def f(zh, zl):
        return log_cdf_diff(zh, zl)

    # Moderately large z values
    zh = jnp.array(6.0)
    zl = jnp.array(5.0)
    val = f(zh, zl)
    grad_zh, grad_zl = jax.grad(f, argnums=(0, 1))(zh, zl)
    assert jnp.isfinite(val)
    assert jnp.isfinite(grad_zh)
    assert jnp.isfinite(grad_zl)

    # Moderately negative z values
    zh = jnp.array(-5.0)
    zl = jnp.array(-6.0)
    val = f(zh, zl)
    grad_zh, grad_zl = jax.grad(f, argnums=(0, 1))(zh, zl)
    assert jnp.isfinite(val)
    assert jnp.isfinite(grad_zh)
    assert jnp.isfinite(grad_zl)
