...
def simulate_internal(
    pomp_obj=None, rinit=None, rprocess=None, rmeas=None, ys=None, theta=None, 
    time_vec=None, covars=None, Nsim=100, state_names=None, 
    key=jax.random.PRNGKey(123), format = "array", plot=False
):
    # TODO standardize arguments as rinit, rproc, rmeas
    ...
    if pomp_obj is not None:
        ...
        rmeas = pomp_obj.rmeas
        ys = pomp_obj.ys
        theta = pomp_obj.theta
        covars = pomp_obj.covars
        rinitial = jax.vmap(rinit, (0, None, 0, None))
        rprocess = jax.vmap(rproc, (0, None, 0, None))
        rmeasure = jax.vmap(rmeas, (0, None, 0, None))
    ...
    initial_state = rinitial(J, theta, keys, covars)
    ...
    for i in range (ylen):
        ...
        key, *keys = jax.random.split(key, num = J + 1)
        measurements = rmeasure(particles, theta, keys, covars)
        state_list.append(particles)
    ...
    if plot is True:
        if time_vec is None:
            ...
            print(
                "Error in time vector: 'time_vec' should have the same length "
                "as 'ys'."
            )
        ...
        for state in range(latent_states):
            ...
            for j in range(J):
                (
                    particle_values
                ) = [state_list[t][j, state] for t in range(ylen + 1)]
                ax.plot(time, particle_values, alpha=0.7) 
            
            ax.plot(
                time, mean[:, state], color='black', linewidth=2, label='Mean'
            )
            ax.plot(
    ...
    return (state_list, measurement_list)
