...
def pfilter_pf(pomp_object=None, J=50, rinit=None, rprocess=None, dmeasure=None, theta=None,\
               ys=None, covars=None, thresh=100, key=None):
    """
    An outside function for calculating the mean result using particle filtering algorithm 
    with weight equalization across measurements. It receives two kinds of input - pomp class 
    object or the required arguments of 'pfilter_pf_internal' function, and executes on the object
    or calls 'pfilter_pf_internal' directly.

    Args:
        pomp_object (Pomp, optional): An instance of the POMP class. If provided, the function will
        execute on this object to conduct the particle filtering algorithm. Defaults to None.
    ...
    if rinit is not None and rprocess is not None and dmeasure is not None and theta is not None\
           and ys is not None:
