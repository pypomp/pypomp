...
class Pomp:
    ...
    def perfilter_mean(self, J, sigmas, a = 0.9,thresh=100, key=None):
        ...
        return perfilter_internal_mean(self.theta, self.ys, J, sigmas, self.rinit, self.rprocesses, self.dmeasures, ndim = self.theta.ndim, covars = self.covars, a = a, thresh = thresh, key = key)
    ...
    def fit(self, sigmas = None, sigmas_init = None, M = 10, a = 0.9, 
        ...
        return fit_internal(self.theta, self.ys, self.rinit, self.rprocess, self.dmeasure, self.rprocesses, self.dmeasures, sigmas, sigmas_init, self.covars, M = M, a = a, J = J, Jh = Jh, method = method, itns = itns, beta = beta, eta = eta, c = c, max_ls_itn = max_ls_itn, thresh_mif = thresh_mif, 
                           thresh_tr = thresh_tr, verbose = verbose, scale = scale, ls = ls, alpha = alpha, monitor = monitor, mode = mode)