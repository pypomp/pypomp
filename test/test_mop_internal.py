...
from pypomp.LG import get_thetas
...
class TestMopInternal_LG(unittest.TestCase):
    def setUp(self):
        ...
        self.J = 10
        self.ys = LG_obj.ys
        self.theta = LG_obj.theta
        self.covars = LG_obj.covars
        self.key = jax.random.key(111)

        self.rinit = LG_obj.rinit.struct
        self.rproc = LG_obj.rproc.struct
        self.dmeas = LG_obj.dmeas.struct
        self.rprocess = LG_obj.rproc.struct_pf
        self.dmeasure = LG_obj.dmeas.struct_pf
        self.rprocesses = LG_obj.rproc.struct_per
        self.dmeasures = LG_obj.dmeas.struct_per

    def test_basic(self):
        result1 = _mop_internal(
            self.theta,
            self.ys,
            self.J,
            self.rinit,
