...
def perfilter(pomp_object=None, J=50, rinit=None, rprocesses=None, dmeasures=None, theta=None, 
              ys=None, sigmas=None, covars=None, thresh=100, key=None):
    """
    An outside function for perturbed particle filtering algorithm. It receives two kinds of 
    input - pomp class object or the required arguments of 'perfilter_internal' function, and 
    executes on the object or calls 'perfilter_internal' directly.

    Args:
        pomp_object (Pomp, optional): An instance of the POMP class. If provided, the function 
                                      will execute on this object to conduct the perturbed 
        ...
        dmeasures (function, optional): Density evaluation for the measurement model. 
                                        Defaults to None.
    ...
    if rinit is not None and rprocesses is not None and dmeasures is not None and theta\
            is not None and ys is not None and sigmas is not None:
        return _perfilter_internal(theta=theta, ys=ys, J=J, sigmas=sigmas, rinit=rinit, 
                                   rprocesses=rprocesses, dmeasures=dmeasures, 
                                   ndim=theta.ndim, covars=covars, thresh=thresh, key=key)
    else:
