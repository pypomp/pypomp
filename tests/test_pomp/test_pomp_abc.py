...
class TestABC:
    ...
    def test_to_dataframe(self):
        ...
        sir.abc(
            Nabc=3, probes=_default_probes(), scale=_default_scale(),
            epsilon=1e6, proposal=prop, key=jax.random.key(4),
        )
    ...
    def test_with_adaptive_proposal(self):
        ...
        sir.abc(
            Nabc=6, probes=_default_probes(), scale=_default_scale(),
            epsilon=1e6, proposal=prop, key=jax.random.key(8),
        )
        assert isinstance(sir.results_history[-1], PompABCResult)
    ...
    def test_tight_epsilon_low_acceptance(self):
        ...
        sir.abc(
            Nabc=10, probes=_default_probes(), scale=_default_scale(),
            epsilon=1.0, proposal=prop, key=jax.random.key(9),
        )
        res = sir.results_history[-1]
...
class TestABCValidation:
    ...
    def test_invalid_epsilon(self):
        ...
        with pytest.raises(ValueError, match="epsilon"):
            sir.abc(
                Nabc=5, probes=_default_probes(), scale=_default_scale(),
                epsilon=-1.0, proposal=prop, key=jax.random.key(0),
            )
...
class TestABCMerge:
    def test_merge_two_chains(self):
        ...
        sir.abc(
            Nabc=3, probes=_default_probes(), scale=_default_scale(),
            epsilon=1e6, proposal=prop, key=jax.random.key(20),
        )
        res1 = sir.results_history[-1]
        sir.abc(
            Nabc=3, probes=_default_probes(), scale=_default_scale(),
            epsilon=1e6, proposal=prop, key=jax.random.key(21),
        )
    ...
    def test_merge_different_epsilon_raises(self):
        ...
        sir.abc(
            Nabc=3, probes=_default_probes(), scale=_default_scale(),
            epsilon=1e6, proposal=prop, key=jax.random.key(30),
        )
        res1 = sir.results_history[-1]
        sir.abc(
            Nabc=3, probes=_default_probes(), scale=_default_scale(),
            epsilon=2e6, proposal=prop, key=jax.random.key(31),
        )
        res2 = sir.results_history[-1]
        with pytest.raises(ValueError, match="epsilon"):
