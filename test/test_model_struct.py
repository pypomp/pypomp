...
class TestModelStruct(unittest.TestCase):
    ...
    def test_RProc_value_error(self):
        # Test that an error is thrown with incorrect arguments
        with self.assertRaises(ValueError):
            pp.RProc(lambda foo, theta_, key, covars, t: jnp.array([0]))
        with self.assertRaises(ValueError):
            pp.RProc(lambda X_, foo, key, covars, t: jnp.array([0]))
        with self.assertRaises(ValueError):
            pp.RProc(lambda X_, theta_, foo, covars, t: jnp.array([0]))
        with self.assertRaises(ValueError):
            pp.RProc(lambda X_, theta_, key, foo, t: jnp.array([0]))
        with self.assertRaises(ValueError):
            pp.RProc(lambda X_, theta_, key, covars, foo: jnp.array([0]))
        # Test that correct arguments run without error
        pp.RProc(lambda X_, theta_, key, covars, t: jnp.array([0]))
    ...
    def test_RMeas_value_error(self):
        # Test that an error is thrown with incorrect arguments
        with self.assertRaises(ValueError):
            pp.RMeas(lambda foo, theta_, key, covars, t: jnp.array([0]))
        with self.assertRaises(ValueError):
            pp.RMeas(lambda X_, foo, key, covars, t: jnp.array([0]))
        with self.assertRaises(ValueError):
            pp.RMeas(lambda X_, theta_, foo, covars, t: jnp.array([0]))
        with self.assertRaises(ValueError):
            pp.RMeas(lambda X_, theta_, key, foo, t: jnp.array([0]))
        with self.assertRaises(ValueError):
            pp.RMeas(lambda X_, theta_, key, covars, foo: jnp.array([0]))
        # Test that correct arguments run without error
        pp.RMeas(lambda X_, theta_, key, covars, t: jnp.array([0]))
