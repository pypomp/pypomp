...
class TestFit_LG(unittest.TestCase):
    ...
    def test_internal_mif_basic(self):
        mif_loglik1, mif_theta1 = fit(J = self.J, Jh = 10, theta = self.theta, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure, rprocesses = self.rprocesses, dmeasures = self.dmeasures, 
        ...
        mif_loglik2, mif_theta2 = fit(theta = self.theta, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure, rprocesses = self.rprocesses, dmeasures = self.dmeasures, 
    ...
    def test_class__mif_basic(self):
        ...
        mif_loglik1, mif_theta1 = fit(pomp_obj, J = self.J, Jh = 10, sigmas = 0.02, sigmas_init = 1e-20, M = 2, a = 0.9, thresh_mif = -1, mode = "IF2")
        self.assertEqual(mif_loglik1.shape, (3,))
        self.assertEqual(mif_theta1.shape, (3, self.J, ) + self.theta.shape)
        self.assertTrue(np.issubdtype(mif_loglik1.dtype, np.float32))
        self.assertTrue(np.issubdtype(mif_theta1.dtype, np.float32))
    ...
    def test_invalid_mif_input(self):
        ...
        with self.assertRaises(ValueError) as text:
            fit(J = self.J, theta = self.theta, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure, rprocesses = self.rprocesses, dmeasures = self.dmeasures, 
    ...
    def test_internal_GD_basic(self):
        ## ls = False
        #method = SGD
        GD_loglik1, GD_theta1 = fit(J = self.J, Jh = 10, theta = self.theta, ys = self.ys, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure,  itns = 2, method = "SGD", alpha = 0.97, scale = True, mode = "GD")
        ...
        #method = Newton
        GD_loglik3, GD_theta3 = fit(J = self.J, Jh = 10, theta = self.theta, ys = self.ys, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure,  itns = 2, method = "Newton", alpha = 0.97, scale = True, mode = "GD")
        ...
        #refined Newton
        GD_loglik5, GD_theta5 = fit(J = self.J, Jh = 10, theta = self.theta, ys = self.ys, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure,  itns = 2, method = "WeightedNewton", alpha = 0.97, scale = True, mode = "GD")
        ...
        #BFGS
        GD_loglik7, GD_theta7 = fit(J = self.J, Jh = 10, theta = self.theta, ys = self.ys, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure,  itns = 2, method = "BFGS", alpha = 0.97, scale = True, mode = "GD")
    ...
    def test_class_GD_basic(self):
        ...
        ## ls = True
        GD_loglik1, GD_theta1 = fit(pomp_obj, J = self.J, Jh = 10, itns = 2, method = "SGD", alpha = 0.97, scale = True, ls = True, mode = "GD")
        self.assertEqual(GD_loglik1.shape, (3,))
        self.assertEqual(GD_theta1.shape, (3,) + self.theta.shape)
        self.assertTrue(np.issubdtype(GD_loglik1.dtype, np.float32))
        self.assertTrue(np.issubdtype(GD_theta1.dtype, np.float32))
    
        GD_loglik2, GD_theta2 = fit(pomp_obj, J = self.J, Jh = 10, itns = 2, method = "Newton", alpha = 0.97, scale = True, ls = True, mode = "GD")
        ...
        GD_loglik3, GD_theta3 = fit(pomp_obj, J = self.J, Jh = 10, itns = 2, method = "WeightedNewton", alpha = 0.97, scale = True, ls = True, mode = "GD")
        self.assertEqual(GD_loglik3.shape, (3,))
        self.assertEqual(GD_theta3.shape, (3,) + self.theta.shape)
        self.assertTrue(np.issubdtype(GD_loglik3.dtype, np.float32))
        self.assertTrue(np.issubdtype(GD_theta3.dtype, np.float32))
    ...
    def test_invalid_GD_input(self):
        ...
        with self.assertRaises(ValueError) as text:
            fit(rinit = self.rinit, rprocesses = self.rprocesses, dmeasures = self.dmeasures, theta = self.theta, ys = self.ys, mode = "GD")
        ...
        with self.assertRaises(ValueError) as text:
           fit(J = self.J, Jh = 10, theta = self.theta, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure, ys = self.ys, itns = 2, alpha = 0.97, scale = True, mode = "SGD")
    ...
    def test_internal_IFAD_basic(self):
        ...
        IFAD_loglik1, IFAD_theta1 = fit(J = self.J, Jh = 10, theta = self.theta, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure, rprocesses = self.rprocesses, dmeasures = self.dmeasures,
                                        ys = self.ys, sigmas = self.sigmas, sigmas_init = 1e-20, M = 2, a = 0.9, method = "SGD", itns = 2, ls = True, alpha = 0.97, mode = "IFAD")
        ...
        IFAD_loglik2, IFAD_theta2 = fit(J = self.J, Jh = 10, theta = self.theta, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure, rprocesses = self.rprocesses, dmeasures = self.dmeasures,
                                        ys = self.ys, sigmas = self.sigmas, sigmas_init = 1e-20, M = 2, a = 0.9, method = "Newton", itns = 2, ls = True, alpha = 0.97, mode = "IFAD")
        ...
        IFAD_loglik3, IFAD_theta3 = fit(J = self.J, Jh = 10, theta = self.theta, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure, rprocesses = self.rprocesses, dmeasures = self.dmeasures,
                                        ys = self.ys, sigmas = self.sigmas, sigmas_init = 1e-20, M = 2, a = 0.9, method = "WeightedNewton", itns = 2, ls = True, alpha = 0.97, mode = "IFAD")
        ...
        IFAD_loglik4, IFAD_theta4 = fit(J = self.J, Jh = 10, theta = self.theta, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure, rprocesses = self.rprocesses, dmeasures = self.dmeasures,
                                        ys = self.ys, sigmas = self.sigmas, sigmas_init = 1e-20, M = 2, a = 0.9, method = "BFGS", itns = 2, ls = True, alpha = 0.97, mode = "IFAD")
    ...
    def test_class_IFAD_basic(self):
        ...
        IFAD_loglik1, IFAD_theta1 = fit(pomp_obj, J = self.J, sigmas = self.sigmas, sigmas_init = 1e-20, M = 2, a = 0.9, method = "SGD", itns = 2, alpha = 0.97, mode = "IFAD")
        self.assertEqual(IFAD_loglik1.shape, (3,))
        self.assertEqual(IFAD_theta1.shape, (3,) + self.theta.shape)
        self.assertTrue(np.issubdtype(IFAD_loglik1.dtype, np.float32))
        self.assertTrue(np.issubdtype(IFAD_theta1.dtype, np.float32))

        IFAD_loglik2, IFAD_theta2 = fit(pomp_obj, J = self.J, sigmas = self.sigmas, sigmas_init = 1e-20, M = 2, a = 0.9, method = "Newton", itns = 2, alpha = 0.97, mode = "IFAD")
        self.assertEqual(IFAD_loglik2.shape, (3,))
        self.assertEqual(IFAD_theta2.shape, (3,) + self.theta.shape)
        self.assertTrue(np.issubdtype(IFAD_loglik2.dtype, np.float32))
        self.assertTrue(np.issubdtype(IFAD_theta2.dtype, np.float32))
        
        IFAD_loglik3, IFAD_theta3 = fit(pomp_obj, J = self.J, sigmas = self.sigmas, sigmas_init = 1e-20, M = 2, a = 0.9, method = "WeightedNewton", itns = 2, alpha = 0.97, mode = "IFAD")
        self.assertEqual(IFAD_loglik3.shape, (3,))
        self.assertEqual(IFAD_theta3.shape, (3,) + self.theta.shape)
        self.assertTrue(np.issubdtype(IFAD_loglik3.dtype, np.float32))
        self.assertTrue(np.issubdtype(IFAD_theta3.dtype, np.float32))

        IFAD_loglik4, IFAD_theta4 = fit(pomp_obj, J = self.J, sigmas = self.sigmas, sigmas_init = 1e-20, M = 2, a = 0.9, method = "BFGS", itns = 2, alpha = 0.97, mode = "IFAD")
        self.assertEqual(IFAD_loglik4.shape, (3,))
        self.assertEqual(IFAD_theta4.shape, (3,) + self.theta.shape)
        self.assertTrue(np.issubdtype(IFAD_loglik4.dtype, np.float32))
        self.assertTrue(np.issubdtype(IFAD_theta4.dtype, np.float32))
    ...
    def test_invalid_IFAD_input(self):
        ...
        with self.assertRaises(ValueError) as text:
            fit(pomp_obj, J = self.J, sigmas = self.sigmas, sigmas_init = 1e-20, M = 2, a = 0.9, method = "SGD", itns = 2, alpha = 0.97, mode = "AD")
        ...
        with self.assertRaises(ValueError) as text:
            ...
            fit(J = self.J, Jh = 10, theta = self.theta, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure, rprocesses = self.rprocesses, dmeasures = self.dmeasures, 
                ys = self.ys, sigmas = self.sigmas, sigmas_init = 1e-20, M = 2, a = 0.9, method = "SGD", itns = 2, ls = True, alpha = 0.97, mode = "AD")
        ...
        with self.assertRaises(ValueError) as text:
            fit(J = self.J, Jh = 10, theta = self.theta, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure, rprocesses = self.rprocesses, dmeasures = self.dmeasures, 
                ys = self.ys, sigmas = self.sigmas, M = 2, a = 0.9, method = "WeightedNewton", itns = 2, ls = True, alpha = 0.97, mode = "IFAD")
        self.assertEqual(str(text.exception), "Invalid Argument Input with Missing workhorse or sigmas")
        ...
        with self.assertRaises(ValueError) as text:
            fit(J = self.J, Jh = 10, theta = self.theta, rinit = self.rinit, rprocesses = self.rprocesses, dmeasures = self.dmeasures, 
                ys = self.ys, sigmas = self.sigmas, sigmas_init = 1e-20, M = 2, a = 0.9, method = "Newton", itns = 2, ls = True, alpha = 0.97, mode = "IFAD")
