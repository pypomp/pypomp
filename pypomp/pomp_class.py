...
class Pomp:
    ...
    def mop(self, J, alpha=0.97, key=None):
        """
        Instance method for MOP algorithm, which uses the initialized instance parameters
        and calls 'mop_internal' function.
    ...
    def mop_mean(self, J, alpha=0.97, key=None):
        """
        Instance method for calculating the mean result using MOP algorithm across 
        the measurements, which uses the initialized instance parameters and calls 
        ...
        return _mop_internal_mean(self.theta, self.ys, J, self.rinit, self.rprocess, 
                                  self.dmeasure, self.covars, alpha, key)

    def pfilter(self, J, thresh=100, key=None):
        """
        Instance method for particle filtering algorithm, which uses the initialized instance
        parameters and calls 'pfilter_internal' function.
        ...
        return _pfilter_internal(self.theta, self.ys, J, self.rinit, self.rprocess,
                                self.dmeasure, self.covars, thresh, key)

    def pfilter_mean(self, J, thresh=100, key=None):
        """
        Instance method for calculating the mean result using particle filtering algorithm 
        across the measurements, which uses the initialized instance parameters and calls 
        ...
        return _pfilter_internal_mean(self.theta, self.ys, J, self.rinit, self.rprocess,
                                      self.dmeasure, self.covars, thresh, key)

    def perfilter(self, J, sigmas, thresh=100, key=None):
        """
        Instance method for perturbed particle filtering algorithm, which uses the initialized 
        instance parameters and calls "perfilter_internal" function.
    ...
    def perfilter_mean(self, J, sigmas, thresh=100, key=None):
        """
        Instance method for calculating the mean result using perturbed particle filtering
        algorithm across the measurements, which uses the initialized instance parameters 
        and calls 'perfilter_internal_mean' function.
        ...
        return _perfilter_internal_mean(self.theta, self.ys, J, sigmas, self.rinit, 
                                        self.rprocesses, self.dmeasures, ndim=self.theta.ndim, 
                                        covars=self.covars, thresh=thresh, key=key)

    def pfilter_pf(self, J, thresh=100, key=None):
        """
        Instance method for calculating the mean result using particle filtering algorithm with 
        weight equalization across the measurements, which uses the initialized instance 
        parameters and calls 'pfilter_pf_internal' function.
        ...
        Returns:
            float: The mean of negative log-likelihood value across the measurements.
        """
        return _pfilter_pf_internal(self.theta, self.ys, J, self.rinit, self.rprocess, 
                                    self.dmeasure, self.covars, thresh, key)
    ...
    def mif(self, sigmas, sigmas_init, M=10, a=0.9, J=100, thresh=100, monitor=False,
        ...
        Args:
            ...
            thresh (float, optional): Threshold value to determine whether to resample 
                                      particles. Defaults to 100.
            monitor (bool, optional): Boolean flag controlling whether to monitor the 
                                      log-likelihood value. Defaults to False.
    ...
    def train(self, theta_ests, J=5000, Jh=1000, method='Newton', itns=20, beta=0.9, 
        ...
        Args:
            ...
            J (int, optional): The number of particles in the MOP objective for obtaining 
                               the gradient. Defaults to 5000.
            Jh (int, optional): The number of particles in the MOP objective for obtaining 
                                the Hessian matrix. Defaults to 1000.
            method (str, optional): The gradient-based iterative optimization method to use, 
                                    including Newton method, weighted Newton method BFGS 
                                    method, gradient descent. Defaults to 'Newton'.
            ...
            thresh (int, optional): Threshold value to determine whether to resample particles 
                                    in pfilter function. Defaults to 100.
            verbose (bool, optional): Boolean flag controlling whether to print out the 
                                      log-likelihood and parameter information. 
                                      Defaults to False.
    ...
    def fit(self, sigmas=None, sigmas_init=None, M=10, a=0.9,
        ...
        differentiation (IFAD), which uses the initialized instance parameters and calls 
        ...
        Args:
            ...
            J (int, optional): The number of particles in iterated filtering and the number of 
                               particles in the MOP objective for obtaining the gradient in 
                               gradient-based optimization procedure. Defaults to 100.
            Jh (int, optional): The number of particles in the MOP objective for obtaining the 
                                Hessian matrix. Defaults to 1000.
            method (str, optional): The gradient-based iterative optimization method to use, 
                                    including Newton method, weighted Newton method, BFGS 
                                    method and gradient descent. Defaults to 'Newton'.
            ...
            max_ls_itn (int, optional): The maximum number of iterations for the line search
                                        algorithm. Defaults to 10.
            thresh_mif (int, optional): Threshold value to determine whether to resample
                                        particles in iterated filtering. Defaults to 100.
            thresh_tr (int, optional): Threshold value to determine whether to resample 
                                       particles in gradient optimization. Defaults to 100.
            verbose (bool, optional):  Boolean flag controlling whether to print out the 
                                       log-likelihood and parameter information. 
                                       Defaults to False.
            scale (bool, optional): Boolean flag controlling normalizing the direction or not. 
            ...
            monitor (bool, optional): Boolean flag controlling whether to monitor the 
                                      log-likelihood value. Defaults to True.
            mode (str, optional): The optimization algorithm to use, including 'IF2', 'GD', 
                                  and 'IFAD'. Defaults to "IFAD".
        ...
        return _fit_internal(self.theta, self.ys, self.rinit, self.rprocess, self.dmeasure, 
