...
class TestMop_LG(unittest.TestCase):
    ...
    def test_internal_basic(self):
        val1 = mop(J = self.J, rinit = self.rinit, rprocess = self.rprocess, dmeasure = self.dmeasure, theta = self.theta, ys = self.ys, alpha = 0.97, key = self.key )
    ...
    def test_class_basic(self):
        ...
        val1 = mop(pomp_obj, self.J, alpha = 0.97, key = self.key)
        val2 = mop(pomp_obj)
        val3 = mop(pomp_obj, self.J, self.rinit, self.rprocess, self.dmeasure, theta = [], ys = [])
        self.assertEqual(val1.shape, ())
        self.assertTrue(np.isfinite(val1.item()))
        self.assertEqual(val1.dtype, np.float32)
        self.assertEqual(val2.shape, ())
