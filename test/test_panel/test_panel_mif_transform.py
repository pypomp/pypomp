...
def panel_pomp_with_transform():
    ...
    # Create panel with shared and unit-specific parameters
    shared_params = pd.DataFrame(
        index=pd.Index(["A1", "C1"]),
        data={"shared": [0.5, 0.5]}
    )
    
    unit_specific_params = pd.DataFrame(
        index=pd.Index(["Q1", "R1"]),
        data={"unit1": [0.1, 0.1], "unit2": [0.2, 0.2]}
    )
...
def test_panel_mif_traces_transformed(panel_pomp_with_transform):
    ...
    shared_names = panel.canonical_shared_param_names if hasattr(panel, 'canonical_shared_param_names') else ["A1", "C1"]
    unit_names = panel.canonical_unit_param_names if hasattr(panel, 'canonical_unit_param_names') else ["Q1", "R1"]
    ...
    if "shared_traces" in results:
        ...
        for param in shared_names:
            if param.startswith('Q') or param.startswith('R'):
                ...
                assert np.all(param_values > 0), f"Shared parameter {param} should be positive"
    ...
    if "unit_traces" in results:
        ...
        for param in unit_names:
            if param.startswith('Q') or param.startswith('R'):
                ...
                param_values = param_values[~np.isnan(param_values)]
                assert np.all(param_values > 0), f"Unit parameter {param} should be positive"
