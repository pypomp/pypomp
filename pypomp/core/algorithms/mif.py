...
from .contexts import MifContext, PfilterContext
from .helpers import _no_resampler_thetas, _normalize_weights, _resampler_thetas
...
def _mif_internal(
    ...
    init_carry = theta_Jd
    scan_xs = (jnp.arange(context.M), m_keys)
...
def _perfilter_internal(
    ...
    time_indices = jnp.arange(len(context.series.ys))
    cooling_factors = jax.vmap(
        lambda i: context.rw_sigma.cooling_factor(
            i, m_current, len(context.series.times)
        )
    )(time_indices)
    # Ancestry tracking is for PIF (Panel.mif with block=False).
    if context.return_ancestry:
    ...
    sigmas_init_cooled = (
        context.rw_sigma.cooling_factor(0, m_current, len(context.series.times))
        * context.rw_sigma.sigmas_init_array
    )
    ...
    particlesF_Jx = context.per.rinitializer(
        thetas_Jd, keys, covars0, context.series.t0, SHOULD_TRANS
    )

    # 4. Prepare scan inputs
    all_keys = jax.random.split(key, num=len(context.series.ys) + 1)
    step_keys_raw = all_keys[1:]
    perfilter_scan_xs = PerfilterStepInputs(
        y=context.series.ys,
        time=context.series.times,
        nstep=context.series.nstep_array,
    ...
    scan_body_fn = jax.tree_util.Partial(_perfilter_scan_body, context)
...
def _perfilter_scan_body(
    ...
    sigmas_cooled = xs.cooling_factor * context.rw_sigma.sigmas_array
    ...
    covars_t = (
        None
        if context.series.covars_extended is None
        else context.series.covars_extended[updated_t_idx]
    )
...
_jv_mif_internal = jit(_vmapped_mif_internal)
