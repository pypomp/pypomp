...
def _jgrad_pf(
    ...
    """
    calculates the gradient of a mean particle filter objective with weight 
    equalization (function 'pfilter_pf_internal') w.r.t. the current estimated 
    ...
    Returns:
        array-like: the gradient of the function pfilter_pf_internal() w.r.t. 
            theta_ests.
    """
    return jax.grad(_pfilter_pf_internal)(
...
def _jvg_pf(
    ...
    objective with weight equalization (function 'pfilter_pf_internal') w.r.t. 
    ...
    Returns:
        ...
        - The mean of negative log-likelihood value across the measurements 
            using pfilter_pf_internal().
        - The gradient of the function pfilter_pf_internal() w.r.t. theta_ests.
    """
    return jax.value_and_grad(_pfilter_pf_internal)(
        theta_ests, ys, J, rinit, rprocess, dmeasure, 
        covars=covars, thresh=thresh, key=key
    )
...
def _train_internal(
    ...
    Args:
        ...
        method (str, optional): The optimization method to use, including 
            Newton method, weighted Newton method, BFGS method, 
            gradient descent. Defaults to 'gradient descent'.
