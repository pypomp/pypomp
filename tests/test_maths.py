...
def test_val():
    ...
    lme = pp.maths.logmeanexp(x)
    lme_se = pp.maths.logmeanexp_se(x)
...
def test_logmeanexp_edge_cases(arr, expect_lme_nan, expect_se_nan):
    with warnings.catch_warnings():
        ...
        lme = pp.maths.logmeanexp(arr)
        lme_se = pp.maths.logmeanexp_se(arr)
...
def test_large_values():
    ...
    expected = 1e10 + pp.maths.logmeanexp(np.array([0, 1, 2]))
...
def test_ignore_nan(arr, expected_arr, expect_nan):
    with warnings.catch_warnings():
        warnings.simplefilter("ignore", UserWarning)
        lme = pp.maths.logmeanexp(arr, ignore_nan=True)
        lme_se = pp.maths.logmeanexp_se(arr, ignore_nan=True)

    if expect_nan:
        assert np.isnan(lme)
        assert np.isnan(lme_se)
    else:
        expected_lme = pp.maths.logmeanexp(expected_arr)
